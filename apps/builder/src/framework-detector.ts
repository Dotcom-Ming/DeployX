...
export class FrameworkDetector {
  ...
  async detectFramework(projectDir: string): Promise<DetectedFramework> {
    ...
    // Check for static site (index.html without package.json or with no framework deps)
    ...
    } else if (hasIndexHtml && Object.keys(deps).length === 0) {
    ...
    // Allow package.json scripts to override defaults
    const scripts = packageJson.scripts || {};
    const buildCmd =
      scripts['build'] && framework !== Framework.STATIC && framework !== Framework.NODE
        ? `npm run build`
        : config.buildCmd;
